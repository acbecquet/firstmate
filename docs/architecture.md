...
For ship and scout work, `fm-spawn.sh` waits for `treehouse get` and then refuses to launch unless the pane resolves to a real git worktree root of the project repo - one sharing the project clone's git common dir - that is distinct from the project primary checkout, the firstmate primary checkout, and the firstmate home.
A pane-cwd startup transient parked in a foreign git toplevel (such as the tmux server's own cwd, which for firstmate's session is its primary checkout) is polled past rather than latched, so it never lands in `state/<id>.meta` or gets a turn-end hook.
...
`fm-home-seed.sh` provisions the isolated home, clones the listed PR-based projects into it, initializes newly cloned `no-mistakes` projects, copies the charter to `data/charter.md`, copies a non-empty `config/crew-model` pin from the seeding firstmate, and `fm-spawn.sh --secondmate` launches it through the same tmux and status-file path as any direct report.
...
Hub-driven spin-up closes the loop: when a secondmate's `machine=` (meta) or `machine:` (registry) names a non-hub box, `fm-spawn.sh --secondmate` takes a remote path that opens the `fm-<id>` window in the box's registry `tmux-session` and launches the harness under `claude remote-control` over the transport, seeds the box's `data/charter.md` from the hub-side charter brief and confirms it landed, records `machine=`/`host=`/`remote_home=` in the hub-side meta, delivers the charter pointer, and arms status carry-back; an empty/`hub` machine stays the byte-for-byte local launch.
The reachability probe `fm-machine-ping.sh` keeps the registry `status:`/`last-seen` honest (run at bootstrap and on the heartbeat, cheap and non-fatal), routing work to an offline box through an `awaiting-machine` backlog blocker instead of a hang, and cross-machine self-update advances a `machine:`-tagged box clone over the transport with the same guarded origin fast-forward (`fm-ff-lib.sh`, driven by `fm-update.sh` and the spawn pre-launch sync).
Absent tags mean today's local behavior, so these fields stay inert until used.
