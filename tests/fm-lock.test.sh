...
# Track spawned fake-harness pids so the EXIT trap reaps them. lib.sh installs its
# own fm_test_cleanup EXIT trap on the first fm_test_tmproot call above; override
# it with one that also kills our processes, then delegates to fm_test_cleanup.
FAKE_PIDS=()
cleanup_all() {
  local p
...
# process whose comm/args match fm-lock.sh's harness regex (a script named
# `claude`). Any args (e.g. --resume <sid>) stay visible in its cmdline.
spawn_fake_harness() {
  local dir=$1; shift
