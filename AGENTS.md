...
It then delivers the charter pointer as a from-firstmate request (the booted Remote Control session reads `data/charter.md` from its own box home and operates as that secondmate) and arms status carry-back (`fm-status-pull.sh arm <id>`).
