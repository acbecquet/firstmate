...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, worktree=, project=, harness=, kind=, mode=, yolo=; kind=secondmate also records home= and projects=, and a remote secondmate additionally records machine=, host=, and remote_home= (fm-pr-check appends pr= and verified pr_head= when available)
...
For ship and scout tasks, the script creates the window (in your current tmux session, or a dedicated `firstmate` session when you are outside tmux), runs `treehouse get`, waits for the worktree subshell, asserts the resolved worktree is a genuine isolated worktree of the project repo distinct from the primary checkout (aborting the spawn otherwise, to prevent the worktree tangle of section 8; a pane-cwd startup transient that is a foreign git toplevel, such as the firstmate primary, is polled past rather than recorded), installs the turn-end hook, records `state/<id>.meta`, and launches the agent with the brief.
...
Two further guards prevent the tangle upstream: `fm-spawn` refuses to launch unless `treehouse get` yields a genuine isolated worktree of the project repo (same git common dir as the project clone, never the firstmate primary or home) distinct from the primary checkout, and every ship brief's first instruction has the crewmate verify it is in its own worktree before branching (section 11).
...
**Remote secondmate spin-up - hub-driven launch under Remote Control (`bin/fm-spawn.sh --secondmate`).**
The hub can start a registered secondmate's firstmate session ON its box, not just route to a session a human pre-started.
The branch falls out of the `machine:` field: when a secondmate's `machine=` (its `state/<id>.meta`, authoritative on recovery) or `machine:` (its `data/secondmates.md` line) names a non-hub registered box, `fm-spawn.sh --secondmate` takes the remote path; empty or `hub` is the local path, byte-for-byte unchanged (a mock test proves the local path makes no ssh call and writes no `machine=` meta).
On the remote path the hub does NOT touch a local home or run the local fast-forward - the box owns its home, its version, and its crewmates - it instead works over the transport: it resolves the box's `host`, `tmux-session`, and `harness` from the machine registry, exports `FM_TMUX_SSH` from `fm-machines.sh ssh-prefix`, opens a window `fm-<id>` in the box's registry `tmux-session` rooted at the box's firstmate home, and launches the harness under `claude remote-control --name fm-<id> --permission-mode bypassPermissions` so the captain can ride along from claude.ai/code (only the claude harness has Remote Control today; another box harness is refused, not launched).
It records `machine=`/`host=`/`remote_home=` in the secondmate's meta (the fields that drive `fm-send`/`fm-peek`/`fm-status-pull` routing and recovery), so the spawn, every later steer, and a post-restart respawn all resolve remote-vs-local from the same `machine=` with no extra wiring.
Because a box firstmate home is a fresh clone with an empty, gitignored `data/` (unlike a local secondmate, whose home `fm-home-seed.sh` seeds), the hub first seeds the box's `data/charter.md` over the transport from the hub-side filled charter brief and reads it back to confirm it landed, aborting the spin-up before any box window is opened if the hub-side charter is missing or still holds `{TASK}` or the box write cannot be confirmed.
It then delivers the charter pointer as a from-firstmate request (the booted Remote Control session reads that seeded `data/charter.md` from its own box home and operates as that secondmate) and arms status carry-back (`fm-status-pull.sh arm <id>`).
